...
This module provides functionality to convert HEALPix (Hierarchical Equal Area isoLatitude Pixelization) cell IDs to Shapely Polygons and GeoJSON FeatureCollection.
...
def healpix2geo(healpix_ids):
    """
    Convert HEALPix cell IDs to Shapely geometry objects.

    Accepts a single healpix_id (int) or a list of healpix_ids. For each valid HEALPix cell ID,
    creates a Shapely Polygon representing the grid cell boundaries. Skips invalid or
    error-prone cells.

    Parameters
    ----------
    healpix_ids : int or list of int
        HEALPix cell ID(s) to convert. Can be a single int or a list of ints.
        Example format: 12345

    Returns
    -------
    shapely.geometry.Polygon or list of shapely.geometry.Polygon
        If a single HEALPix cell ID is provided, returns a single Shapely Polygon object.
        If a list of IDs is provided, returns a list of Shapely Polygon objects.
        Each polygon represents the boundaries of the corresponding HEALPix cell.
        ...
        try:
            ...
            while 12 * (4**resolution) <= healpix_id:
                ...
                if resolution > 29:  # Maximum HEALPix resolution
                    raise ValueError(
                        f"Cell ID {healpix_id} is too large for valid HEALPix resolution"
                    )
            ...
            if healpix_id >= max_pixels:
                raise ValueError(
                    f"Cell ID {healpix_id} is out of range for resolution {resolution} (max: {max_pixels - 1})"
                )

            # Get cell boundaries using healpy helper
            # _cellid2boundaries returns a list of boundary arrays, each with shape (4, 2)
            boundaries = _cellid2boundaries(
                [healpix_id], nside, nest=True, as_geojson=False
            )
...
def healpix2geo_cli():
    ...
    parser = argparse.ArgumentParser(
        description="Convert HEALPix cell ID(s) to Shapely Polygons"
    )
    parser.add_argument(
        "healpix",
        nargs="+",
...
def healpix2geojson(healpix_ids):
    """
    Convert HEALPix cell IDs to GeoJSON FeatureCollection.
    ...
    -------
    dict
        A GeoJSON FeatureCollection containing polygon features for each valid HEALPix cell.
        Each feature includes:
        - geometry: Polygon representing the cell boundaries
        - properties: Contains the HEALPix cell ID, resolution level, and cell metadata

    Examples
    --------
    >>> healpix2geojson(12345)
    {'type': 'FeatureCollection', 'features': [...]}

    >>> healpix2geojson([12345, 67890])
    {'type': 'FeatureCollection', 'features': [...]}
    """
    if platform.system() != "Linux":
        raise RuntimeError("HEALPix is only supported on Linux systems")
...
def healpix2geojson_cli():
    ...
    """
    parser = argparse.ArgumentParser(
        description="Convert HEALPix cell ID(s) to GeoJSON"
    )
    parser.add_argument(
        "healpix",
        nargs="+",
